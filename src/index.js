  ...
  render: function () {
    ...
    $.each(tempArray, function (index, value) {
      if (pointer.model.get("status") != value) {
        self.append(
          "<option value='" + value + "'>" + "Move to " + value + "</option>"
        );
      }
    });
    this.$el.append(self);
...
function checker() {
  tasks.fetch({
    success: function () {
      ...
      for(i of ['done', 'progress','todo']){
        console.log(i)
        if(_.contains(tempArray, i)){
          let address = tempArray.findIndex((element)=>{
            return element === i;
          });
          tempArray.splice(address,1);
          tempArray.unshift(i);
        }
      }
